...
struct NotchNotesView: View {
                ...
                ZStack {
                    if isEditingNewNote || selectedNoteId != nil {
                        NoteEditorView(
                            ...
                            onSave: saveNote,
                            onExportMarkdown: exportCurrentDraftAsMarkdown,
                            onCancel: cancelEdit,
                        ...
                        NoteListView(
                            ...
                            onExportMarkdown: exportNoteAsMarkdown,
    ...
    private func exportNoteAsMarkdown(_ note: NoteItem) {
        do {
            _ = try MarkdownExportService.exportNote(
                title: note.title,
                content: note.content,
                imageData: note.getImageData(),
                noteID: note.id
            )
            NSWorkspace.shared.open(MarkdownExportService.directory)
        } catch {
            presentMarkdownExportError(error)
        }
    }

    private func exportCurrentDraftAsMarkdown() {
        do {
            _ = try MarkdownExportService.exportNote(
                title: editorTitle,
                content: editorContent,
    ...
    private func presentMarkdownExportError(_ error: Error) {
...
enum MarkdownExportService {
    private static let exportDirectoryName = "Atoll Markdown"
    ...
    @MainActor static func showDirectory() {
        do {
    ...
    static func exportNote(
        ...
        let markdownTitle = displayTitle.isEmpty ? "Untitled Note" : displayTitle
        let baseName = sanitizedBaseName(markdownTitle)
        let identifierSuffix = noteID.map { "-\(String($0.uuidString.prefix(8)))" } ?? ""
        let markdownURL = directory.appendingPathComponent("\(baseName)\(identifierSuffix).md")

        var markdown = "# \(markdownTitle)\n\n"
        ...
        if let imageData {
            let imageURL = directory.appendingPathComponent("\(baseName)\(identifierSuffix)-image.png")
            guard let bitmap = NSBitmapImageRep(data: imageData),
                  let png = bitmap.representation(using: .png, properties: [:]) else {
                throw CocoaError(.fileReadCorruptFile)
            }
            try png.write(to: imageURL, options: .atomic)
            let link = imageURL.lastPathComponent.addingPercentEncoding(withAllowedCharacters: .alphanumerics)!
            markdown += "\n![Attached image](\(link))\n"
        }
    ...
    private static func sanitizedBaseName(_ title: String) -> String {
        ...
        let cleaned = title
            ...
            .joined(separator: "-")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let compact = cleaned.isEmpty ? "Untitled Note" : cleaned
...
struct NoteListView: View {
    ...
    let onExportMarkdown: (NoteItem) -> Void
    let onSync: () -> Void
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    Label("导出 Markdown", systemImage: "arrow.down.doc.fill")
                ...
                .help("Export the newest note as Markdown")

                Button(action: MarkdownExportService.showDirectory) {
                ...
                .buttonStyle(.plain)
                .help("Open Markdown Folder in Finder")
                        ...
                        LazyVGrid(columns: columns, spacing: 8) {
                            ForEach(sortedNotes) { note in
                                NoteRow(
                                    ...
                                    onExportMarkdown: { onExportMarkdown(note) },
                                    isCompact: useGrid
                                )
...
struct NoteRow: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            .padding(.trailing, isCompact ? 68 : 170) // Reserve space for row actions
...
struct NoteEditorView: View {
    ...
    init(title: Binding<String>, content: Binding<String>, imageData: Binding<Data?>, colorIndex: Binding<Int>, onSave: @escaping () -> Void, onExportMarkdown: @escaping () -> Void, onCancel: @escaping () -> Void, isNew: Bool) {
        self._title = title
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                .buttonStyle(.plain)
                .help("Show Markdown Folder in Finder")
