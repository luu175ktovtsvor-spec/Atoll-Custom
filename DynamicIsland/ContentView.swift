...
struct ContentView: View {
    ...
    @State private var outsideClickLocalMonitor: Any?
        ...
        mainLayoutBase
            ...
            .onChange(of: vm.notchState) { _, newState in
                ...
                if newState == .closed {
                    ...
                    startOutsideClickMonitor()
                    // Keep the terminal-specific monitor for shortcut opens that do not
                    // change `currentView`, where the cursor may never enter the notch.
...
//                .keyboardShortcut("E", modifiers: .command)
    ...
    ///
    /// Terminal's outside-click handling remains explicit so a shortcut-opened terminal
    /// can close even when the cursor never entered the notch.
    ///
    ...
    private func shouldPreventOutsideClickClose() -> Bool {
        coordinator.firstLaunch || hasAnyActivePopovers() || vm.isAutoCloseSuppressed
            || ShelfSelectionModel.shared.isDragging || ClipboardManager.shared.isDraggingItem
